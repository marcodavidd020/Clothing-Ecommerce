
class HomeUI {
  static const double topSellingSectionHeight = 320.0;
  static const double productItemWidth = 180.0;
  static const double productItemHeight = 220.0;
  static const double productItemBorderRadius = 12.0;
  static const double productItemSpacing = 16.0;
  static const double productItemTrailingIconSize = 25.0;

  // small vertical spacer
  static const double smallVerticalSpacer = 10.0;
  // medium vertical spacer
  static const double mediumVerticalSpacer = 15.0;
  // large vertical spacer
  static const double largeVerticalSpacer = 20.0;

  // border radius subcategories
  static const double subcategoriesBorderRadius = 12.0;

  // padding subcategories
  static const double subcategoriesPadding = 16.0;
}
